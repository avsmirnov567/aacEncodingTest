...
- (IBAction)playButtonAction:(id)sender {
    if (_encodedData.length > 0) {
        self.bufferWriter = [[WMCBufferWriter alloc] init];
        ...
        [_buttonPlay setTintColor:[UIColor redColor]];
    }
}
...
- (IBAction)recordButtonAction:(id)sender {
    if (!_isRecording) {
        ...
        _isRecording = YES;
        [_buttonRecord setTintColor:[UIColor redColor]];
    } else {
        _isRecording = NO;
        [_recorder stopRecording];
        [_buttonRecord setTintColor:[UIColor blueColor]];
    }
}
